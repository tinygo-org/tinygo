...
test: wasi-libc check-nodejs-version
	CGO_CPPFLAGS="$(CGO_CPPFLAGS)" CGO_CXXFLAGS="$(CGO_CXXFLAGS)" CGO_LDFLAGS="$(CGO_LDFLAGS)" $(GO) test $(GOTESTFLAGS) -v -x -timeout=1h -buildmode exe -tags "byollvm osusergo" $(GOTESTPKGS)
