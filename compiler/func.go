...
func ValidateWasmFunctionParameters(mod llvm.Module) error {
	for fn := mod.FirstFunction(); !fn.IsNil(); fn = llvm.NextFunction(fn) {
		...
		for _, paramType := range fn.GlobalValueType().ParamTypes() {
			...
			if exceeded {
				return fmt.Errorf("function %s has more than %d WebAssembly parameters after ABI lowering; reduce the number or size of its parameters (exported functions cannot lower aggregate parameters indirectly)", fn.Name(), maxFunctionParams)
			}
		}
	}
	return nil
