...
func main() {
	...
	target := flag.String("target", "", "chip/board name or JSON or YAML target specification file")
	...
	case "targets":
		...
		for _, entry := range entries {
			if !entry.Mode().IsRegular() || !strings.HasSuffix(entry.Name(), ".json") && !strings.HasSuffix(entry.Name(), ".yaml") {
				// Only inspect JSON or YAML files.
				continue
			}
