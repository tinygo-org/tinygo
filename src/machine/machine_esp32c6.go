...
// enableLEDCPeriphClock enables the LEDC peripheral clock via PCR.
func enableLEDCPeriphClock() {
	esp.PCR.SetLEDC_CONF_LEDC_RST_EN(1)
	esp.PCR.SetLEDC_CONF_LEDC_CLK_EN(1)
	esp.PCR.SetLEDC_CONF_LEDC_RST_EN(0)
}
...
func (uart *UART) Configure(config UARTConfig) error {
	...
	case uart.Bus == esp.UART0:
		return uart.configure(config, registerSet{
			...
			uartClockBitMask: 0, // PCR-based clock
			gpioMatrixSignal: 6,
		})
...
func (uart *UART) configure(config UARTConfig, regs registerSet) error {
	...
	// - default clock source: 1=80MHz PLL, 2=FOSC_CLK, 3=XTAL_CLK
	uart.Bus.SetCLK_CONF_SCLK_SEL(1)
	// reset divisor of the divider
	...
	uart.Bus.SetHWFC_CONF_RX_FLOW_EN(0)
...
func initUARTClock(bus *esp.UART_Type, regs registerSet) {
	// On ESP32-C6, use the PCR peripheral for clock enable/reset.
	switch {
	case bus == esp.UART0:
		...
		esp.PCR.UART0_SCLK_CONF.Set(
			(esp.PCR.UART0_SCLK_CONF.Get() & ^uint32(3<<20)) | (1 << 20))
	case bus == esp.UART1:
		// Enable UART1 clock via PCR
		...
		esp.PCR.UART1_SCLK_CONF.Set(
			(esp.PCR.UART1_SCLK_CONF.Get() & ^uint32(3<<20)) | (1 << 20))
	}
...
func (uart *UART) setupPins(config UARTConfig, regs registerSet) {
	...
	// link TX with GPIO signal X (technical reference manual) (this is not interrupt signal!)
...
// GetRNG returns 32-bit random numbers using the ESP32-C6 true random number generator.
