...
func readTransfer(msg any, r *bufio.Reader, onEOF func()) (err error) {
	...
	case realLength == 0:
		t.Body = NoBody
	case realLength > 0:
		t.Body = &body{src: io.LimitReader(r, realLength), closing: t.Close, onHitEOF: onEOF}
	default:
		// realLength < 0, i.e. "Content-Length" not mentioned in header
		if t.Close {
			// Close semantics (i.e. HTTP/1.0)
			t.Body = &body{src: r, closing: t.Close, onHitEOF: onEOF}
		} else {
