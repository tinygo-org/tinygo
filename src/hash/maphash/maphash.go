...
// This is a copy of the Go 1.27 hash/maphash package. Upstream uses
// internal/runtime/maps and parts of internal/abi that TinyGo does not
// implement, so this copy calls the TinyGo runtime hash functions instead.
// The abi.EscapeNonString calls are also removed. They keep a value off a
// stack that can move, and TinyGo does not move stacks.
// See https://github.com/tinygo-org/tinygo/issues/5684
...
// TinyGo memhash returns a 64 bit hash only when uintptr is wider than
// 4 bytes. See memhash in src/runtime/algorithm.go.
const use64BitHash = unsafe.Sizeof(uintptr(0)) > 4
...
//go:linkname runtime_comparablehash runtime.comparablehash
func runtime_comparablehash(v any, seed uintptr) uint32
...
func comparableHash[T comparable](v T, seed Seed) uint64 {
	s := seed.s
	// The runtime hasher gives 32 bits, so always combine two hashes. Each
	// seed half is made odd because the FNV hash gives 0 for a 0 seed.
	// See hash32 in src/runtime/memhash_fnv.go.
	lo := runtime_comparablehash(v, uintptr(uint32(s)|1))
	hi := runtime_comparablehash(v, uintptr(uint32(s>>32)|1))
	return uint64(hi)<<32 | uint64(lo)
}
