...
// This is a copy of src/hash/maphash/maphash_test.go in upstream Go. Removed:
// the tests that need internal packages TinyGo does not have, the stack growth
// checks, the panic message check, and the NaN cases. TinyGo does not move
// stacks, and gives NaN the same hash every time. See hashmapFloat64Hash in
