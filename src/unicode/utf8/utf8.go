...
// ptrSize is the size of a uintptr in bytes.
// The upstream Go formula (4 << (^uintptr(0) >> 63)) only distinguishes 32-bit
// and 64-bit architectures. TinyGo also supports 16-bit targets (e.g. AVR),
// where uintptr is 2 bytes, so we use a formula that handles all three sizes.
...
func word[T string | []byte](s T) uintptr {
	if ptrSize == 2 {
		return uintptr(s[0]) | uintptr(s[1])<<8
	}
	if ptrSize == 4 {
