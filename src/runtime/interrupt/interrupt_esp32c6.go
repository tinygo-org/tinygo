...
// The ESP32-C6 has 31 CPU independent interrupts.
...
func (i Interrupt) Enable() error {
	...
	esp.INTPRI.CPU_INT_ENABLE.SetBits(1 << i.num)

	// Set pulse interrupt type (rising edge detection)
	esp.INTPRI.CPU_INT_TYPE.SetBits(1 << i.num)

	// Set default threshold to defaultThreshold
	reg := (*volatile.Register32)(unsafe.Add(unsafe.Pointer(&esp.INTPRI.CPU_INT_PRI_0), i.num*4))
	reg.Set(defaultThreshold)

	// Reset interrupt before reenabling
	esp.INTPRI.CPU_INT_CLEAR.SetBits(1 << i.num)
	esp.INTPRI.CPU_INT_CLEAR.ClearBits(1 << i.num)
...
func handleInterrupt() {
	...
	if !exception && interruptNumber > 0 {
		...
		reg := (*volatile.Register32)(unsafe.Add(unsafe.Pointer(&esp.INTPRI.CPU_INT_PRI_0), interruptNumber*4))
		...
		if esp.INTPRI.CPU_INT_TYPE.Get()&interruptBit != 0 {
			// this is edge type interrupt
			esp.INTPRI.CPU_INT_CLEAR.SetBits(interruptBit)
			esp.INTPRI.CPU_INT_CLEAR.ClearBits(interruptBit)
		} else {
