...
// This file loads a target specification from a JSON or YAML file.
...
// load reads a target specification from the JSON or YAML in the given io.Reader.
// It may load more targets specified using the "inherits" property.
...
// - a relative or absolute path to custom (project specific) target specification .json or .yaml file;
...
func (spec *TargetSpec) loadFromGivenStr(str string) error {
	path := ""
	if strings.HasSuffix(str, ".json") || strings.HasSuffix(str, ".yaml") {
...
func LoadTarget(options *Options) (*TargetSpec, error) {
	...
	// Successfully loaded this target from a built-in .json or .yaml file. Make sure
