...
// libWasiLibc and libWasiLibcWasip2 build wasi-libc for the wasip1 and wasip2
// targets respectively. They're built from the same wasi-libc source tree but
// with different generated headers (see generateWasiVersionHeader) and
// different bottom-half sources selected: wasip1 uses the cloudlibc socket
// syscalls and accept-wasip1.c, while wasip2 uses the descriptor-table-based
// sockets/stdio implementation (generated by wit-bindgen upstream). They use
// distinct library names since compileopts.Config.LibraryPath keys the build
// cache by name plus the LLVM target triple, and both targets share the same
// triple (wasm32-unknown-wasi).
var libWasiLibc = newWasiLibc("1", "wasi-libc")
var libWasiLibcWasip2 = newWasiLibc("2", "wasi-libc-wasip2")

func newWasiLibc(preview, name string) Library {
	return Library{
		name: name,
		makeHeaders: func(target, includeDir string) error {
			...
			// depending on the WASI preview targeted. It's included (directly
			// or transitively) by most of wasi-libc, including wasi/api.h.
			if err := generateWasiVersionHeader(includeDir, preview); err != nil {
				return err
			}
		...
		librarySources: func(target string, libcNeedsMalloc bool) ([]string, error) {
			...
			// wasip1 uses the cloudlibc socket syscalls (getsockopt.c,
			// recv.c, send.c, shutdown.c) plus accept-wasip1.c. wasip2 uses
			// the descriptor-table-based sockets (accept.c, bind.c,
			// connect.c, listen.c, recv.c, send.c, shutdown.c, socket.c,
			// sockopt.c, tcp.c, udp.c, sockets_utils.c, descriptor_table.c,
			// getsockpeername.c, netdb.c, file.c, file_utils.c) plus
			// wasip2.c/wasip2_stdio.c. Both sets define the same symbol
			...
			bottomHalfSrcExclude := []string{
				"wasip3.c", "wasip3_block_on.c", "wasip3_stdio.c", // wasip3-only, not yet supported
			}
			switch preview {
			case "1":
				bottomHalfSrcExclude = append(bottomHalfSrcExclude,
			...
			case "2":
				bottomHalfSrcExclude = append(bottomHalfSrcExclude, "accept-wasip1.c")
				cloudlibcSockSrcExclude = append(cloudlibcSockSrcExclude,
					"getsockopt.c", "recv.c", "send.c", "shutdown.c")
			default:
				return nil, fmt.Errorf("wasi-libc: unsupported WASI preview %q", preview)
			}
			...
			if preview == "2" {
				// See lib/wasi-libc-wasip2-stub.c.
				sources = append(sources, "../wasi-libc-wasip2-stub.c")
			}
